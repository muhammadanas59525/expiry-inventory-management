...
const HomeCust = () => {
    ...
    const processProductsForDisplays = (products) => {
        // Get featured products (products with rating >= 4.5 or marked as featured)
        const featured = products
            .filter(p => p.rating >= 4.5 || p.featured)
            .slice(0, 5);
    ...
    const fetchAllData = async () => {
        try {
            ...
            if (response.data && response.data.success) {
                ...
                console.log('API products loaded:', products.length);
                ...
                // Extract unique shopkeepers
                const shopkeepers = [...new Set(products.map(product => 
                    JSON.stringify(product.shopkeeperId)
                ))].map(shop => JSON.parse(shop));
                setShopkeepers(shopkeepers);
                
                // Process products for displays
                processProductsForDisplays(products);
            } else {
    ...
    // Shopkeeper filter component
    const ShopkeeperFilter = () => {
        if (!shopkeepers || shopkeepers.length === 0) return null;
        
        return (
            <div className="shopkeeper-filter">
                <h3 className="filter-heading">Filter by Shop</h3>
                <div className="shopkeeper-list">
                    <div 
                        className={`shopkeeper-item ${selectedShopkeeper === null ? 'active' : ''}`}
                        onClick={() => filterByShopkeeper(null)}
                    >
                        All Shops
                    </div>
                    {shopkeepers.map(shop => (
                        <div 
                            key={shop._id}
                            className={`shopkeeper-item ${selectedShopkeeper === shop._id ? 'active' : ''}`}
                            onClick={() => filterByShopkeeper(shop._id)}
                        >
                            {shop.name}
                        </div>
                    ))}
    ...
    // Render the shop info in the product card
    const renderShopInfo = (product) => {
        if (!product.shopkeeper) return null;
        
        return (
            <div className="product-shop-info">
                <span className="shop-label">Shop:</span>
                <span className="shop-name">{product.shopkeeper.name}</span>
            </div>
        );
    };

    // Filter products by shopkeeper
    const filterByShopkeeper = async (shopkeeperId) => {
        try {
            setLoading(true);
            setSelectedShopkeeper(shopkeeperId);
            
            if (!shopkeeperId) {
                // If no shopkeeper is selected, fetch all products
                fetchAllData();
                return;
            }
            
            // Fetch products by shopkeeper ID
            const response = await axiosInstance.get(`/products/shopkeeper/${shopkeeperId}`);
            
            if (response.data && response.data.success && Array.isArray(response.data.data)) {
                const shopkeeperProducts = response.data.data;
                setProducts(shopkeeperProducts);
                
                // Extract unique categories from these products
                const uniqueCategories = ['All', ...new Set(shopkeeperProducts.map(product => product.category))];
                setCategories(uniqueCategories);
                
                // Process products for displays
                processProductsForDisplays(shopkeeperProducts);
            } else {
                console.warn('Unexpected API response format:', response.data);
                setError('Failed to load products from this shop. Please try again later.');
            }
            ...
            // Fallback to all products if shopkeeper filter fails
